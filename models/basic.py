...
class IdentityBasedConv1x1(nn.Conv2d):
    ...
    def __init__(self, channels, groups=1):
        ...
        # 调用父类的构造函数，并将 channels 和 groups 参数传递给父类构造函数，
        # 以初始化父类的属性。这种方式通常用于确保父类的属性正确初始化，而子类可以专注于自身特定的初始化需求
...
class BNAndPadLayer(nn.Module):
    ...
    @property
    # 使用装饰器property 可以让类 BNAndPadLayer直接调用属性 weight
    def weight(self):
